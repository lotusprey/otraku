...
class AnimeCollection extends CollectionProvider with ChangeNotifier {
  //Query settings
  ...
  Future<void> fetchSingleList(String status) async {
    const query = r'''
      query Collection($userId: Int, $status: MediaListStatus, $scoreFormat: ScoreFormat) {
        MediaListCollection(userId: $userId, status: $status, type: ANIME, sort: SCORE_DESC) {
          lists {
            name
            status
            isSplitCompletedList
    ...
    final list = EntryList(
      ...
      entries: (listData['entries'] as List<dynamic>)
          .map((e) => MediaEntry(
                ...
                cover: e['media']['coverImage']['medium'],
                format: e['media']['format'],
                progressMaxString: (e['media']['episodes'] ?? '?').toString(),
                entryUserData: EntryUserData(
                  mediaId: e['mediaId'],
                  type: 'ANIME',
                  progress: e['progress'],
